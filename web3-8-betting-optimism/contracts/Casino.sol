...
// ARTICLZ: https://blog.logrocket.com/build-random-number-generator-blockchain/

//commit/reveal protocol : allows 2 or more people to get a random number using a hash function
// A proposes a random number & generates a hash of it which is shared with everyone =>
// B also proposes a random number & shares it with everyone
// A reveals his random number => can be confirmed by calculating the hash
// The final random number is calculated by taking the XOR of R1 and R2 : R1 ^ R2
// we can evaluate the winner by doing a modolo division of the final random number 
