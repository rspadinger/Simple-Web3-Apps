...
//### to make it more secure, our random number isn't just a random number, but the hash of a random number
const valA = ethers.utils.keccak256(0xbad060a7) // 313 421 8407
const hashA = ethers.utils.keccak256(valA)
const valBwin = ethers.utils.keccak256(0x600d60a7) //161 148 9447
const valBlose = ethers.utils.keccak256(0xbad060a7)
// XOR valA ^ valBwin = 3671916544 % 2 = 1 => B wins
...
describe("Casino", async () => {
    it("Not allow you to propose a zero wei bet", async () => {
        ...
        try {
            //this must revert, because no value is transferred
    ...
    it("Not allow you to accept an already accepted bet", async () => {
        ...
        rcpt = await tx.wait()
        expect(rcpt.events[0].event).to.equal("BetAccepted")
        //### custom error handling with try catch => but, .revertedWith should also work
        try {
            tx = await c.acceptBet(hashA, valBwin, { value: 10 })
            rcpt = await tx.wait()
            expect("this").to.equal("fail")
